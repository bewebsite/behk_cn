...
public partial class _eyoung_GuestComment_JoinUsComment_List : AdminPageBase
{
    ...
    private string getStrWhere()
    {
        ...
        {
            strWhere += " Email like '%" + value + "%' or Name like '%" + value + "%' or CommpanyName like '%" + value + "%' or Mobile like '%" + value + "%'";
    ...
    {
        Model.PageData<Model.JoinUsComment> data = bll.GetList(PageSize, base.PageIndex, getStrWhere(), getSort());
    ...
    {
        string[] fileName = { "ID", "Name", "CommpanyName", "Mobile", "Email", "Intor", "AddTime" };
        string[] title = { "编号", "姓名", "公司名称", "手机号", "邮箱", "回访备注", "申请时间" };
        DataToExcel.Export(fileName, title, "TB_JoinUsComment", "加入我们申请列表_" + DateTime.Now.ToString("yyy-MM-dd"), getStrWhere(), getSort());
    }
}

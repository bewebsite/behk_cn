...
public partial class _eyoung_ENHome_ENHomeParteners_List : AdminPageBase
{
    ...
    {
        Model.PageData<Model.ENHomeParteners> data = bll.GetList(PageSize, base.PageIndex, getStrWhere(), getSort());
    ...
    {
        string[] fileName = { "ID", "Name", "Logo", "GetUrl", "Sort" };
        string[] title = { "编号", "名称", "LOGO", "连接地址", "排序" };
        DataToExcel.Export(fileName, title, "TB_ENHomeParteners", "合作伙伴列表_" + DateTime.Now.ToString("yyy-MM-dd"), getStrWhere(), getSort());
    }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        if (rpList.Items.Count > 0)
        {
            ...
            foreach (string _id in arrids)
            {
                ...
                {
                    sql += "UPDATE TB_ENHomeParteners SET Sort =" + Comm.Help.GetInt(Request.Form[_id]) + " WHERE  ID=" + arr[1] + " ;";
